...
interface ActivateProductDrawerProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onActivate: (data: ActivateCatalogProductDto, branchActivations?: BranchActivation[]) => void;
  isLoading?: boolean;
...
export function ActivateProductDrawer({
  ...
  useEffect(() => {
    ...
    setBranchConfigs(prev => {
      ...
      return {
        ...prev,
        ...Object.fromEntries(newEntries.map(b => [b.id, { enabled: true, variants: {} }])),
      };
    });
  }, [product, branches]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const enabledBranches = branches.filter(b => branchConfigs[b.id]?.enabled !== false);
  ...
  const updateBranchVariantOverride = (branchId: string, variantId: string, priceOverride: string) => {
    setBranchConfigs(prev => ({
      ...prev,
      [branchId]: {
        ...prev[branchId],
    ...
    <Drawer open={isOpen} onOpenChange={() => !isLoading && onClose()} isLoading={isLoading}>
      <DrawerContent size="lg">
        ...
        <div className="flex-1 overflow-y-auto">
          <form id="activate-product-form" onSubmit={handleSubmit} className="space-y-5 mt-4 px-7 pb-7">
            {/* Product Preview */}
            ...
            <div className="space-y-3 border-t pt-4">
              ...
              {branches.length === 0 ? (
                ...
                <div className="space-y-2">
                  {branches.map((branch) => {
                    const config = branchConfigs[branch.id] ?? { enabled: true, variants: {} };
                    ...
                    return (
                      <div key={branch.id} className="border rounded-lg overflow-hidden">
                        <div
                          ...
                          onClick={() => hasMultipleVariants && config.enabled && toggleBranchExpand(branch.id)}
                          style={{ cursor: hasMultipleVariants && config.enabled ? "pointer" : "default" }}
                        >
                          <Switch
                          ...
                          {!hasMultipleVariants && config.enabled && variants.length === 1 && (
                            <div
                            ...
                            >
                              ...
                              <Input
                                type="text"
                                inputMode="decimal"
                                value={config.variants[variants[0].id]?.priceOverride ?? ""}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  if (val === "" || /^\d*\.?\d*$/.test(val)) {
                        ...
                        {hasMultipleVariants && config.enabled && isExpanded && (
                              ...
                              <div key={v.id} className="flex items-center gap-3 px-4 py-2.5 bg-white">
                                ...
                                <div className="relative w-24 shrink-0">
                                  ...
                                  <Input
                                    type="text"
                                    inputMode="decimal"
                                    value={config.variants[v.id]?.priceOverride ?? ""}
                                    onChange={(e) => {
                                      const val = e.target.value;
                                      if (val === "" || /^\d*\.?\d*$/.test(val)) {
